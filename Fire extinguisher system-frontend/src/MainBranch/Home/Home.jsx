...
function Home() {
    ...
    const getFireStatusCountForBranch = (branchId, status) => {
        return fires.filter(fire => fire.branch_id === branchId && fire.status === status).length;
    };
            ...
            <table className='Table-MainBranch'>
                ...
                <tbody>
                    {currentBranches.map(branch => {
                        const totalFires = fires.filter(fire => fire.branch_id === branch.branch_id).length;
                        const reportedCount = getFireStatusCountForBranch(branch.branch_id, "report");
                        const resolvedCount = getFireStatusCountForBranch(branch.branch_id, "complete");
